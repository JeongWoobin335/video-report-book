...
- 영상은 30분까지, 모든 사용자 합쳐 하루 30편까지 받습니다. 결과물은 24시간 뒤 지워지고, 무료 서버가 다시 시작되면 그 전에도 사라질 수 있습니다(리포트는 PDF로 저장해 두세요).
- 누가 말했는지(화자 구분)는 아직 하지 않습니다.
- 긴 영상(수백 MB)은 브라우저의 오디오 디코딩이 메모리를 많이 씁니다. Chromium 계열 외의 브라우저는 충분히 시험하지 못했습니다.
- AI가 쓴 글입니다. 검사 스크립트가 형식과 근거를 확인하지만 내용의 정확성까지 보장하지는 않습니다.
